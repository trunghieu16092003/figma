class CartCheckout extends HTMLElement {
  connectedCallback() {
    ...
    const cartHTML = carts
      .map(
        (item) => `
      <div class="flex md:grid md:grid-cols-6 items-start md:items-center gap-4 border-b pb-4">
        ...
        <div class="flex flex-col md:grid md:grid-cols-5 md:col-span-5 gap-4 w-full">
          <div class="flex flex-col md:col-span-2 md:text-center">
            <h3 class="font-medium text-gray-800"><span class="text-[14px] md:hidden">Tên sản phẩm:</span> ${
              item.name
            }</h3>
            <p class="text-sm text-gray-500">Màu: ${item.color} | Size: ${
          item.size
        }</p>
          </div>
          ...
          <p>
          <span class=" md:hidden text-[14px]">Số lượng: </span> <span> ${
            item.quantity
          }</span>
          </p> 
          </div>
          ...
          <div class="flex items-center md:justify-center">
          <p> <span class="md:hidden text-gray-700 text-[14px]">Giá thành:</span> <span class="text-gray-700">${
            item.price
          }</span></p>
           
          </div>

          <div class="flex items-center text-gray-700 md:justify-center">
          <p>
              <span class="md:hidden text-gray-700 text-[14px]">Thành tiền: </span>
               <span class="text-gray-700 font-bold">${(
                 item.price.replace(/\D/g, "") * item.quantity
      ...
      <section class="py-10">
        <div class=" bg-white ">
          ...
          <div class="hidden md:grid grid-cols-6 gap-4 font-semibold text-gray-700 border-b pb-3 mb-4 text-sm md:text-base">
            ...
            <div class="col-span-2 flex items-center justify-center">Sản phẩm</div>
          ...
          <div class="flex flex-col md:flex-row justify-between items-center mt-10 space-y-4 md:space-y-0">
            <div>
              <span class="text-[14px]">Tổng cộng:</span> 
